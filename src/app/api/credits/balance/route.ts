...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data: internalUser } = await supabaseAdmin
      .from("internal_users").select("user_id").eq("user_id", user.id).maybeSingle();
    if (internalUser) return NextResponse.json({ balance: null, is_internal: true });

    const { data: credits } = await supabaseAdmin
      .from("credits").select("balance, total_purchased, total_used").eq("user_id", user.id).maybeSingle();
